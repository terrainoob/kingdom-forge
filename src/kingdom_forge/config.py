...
def _templates(raw: Any) -> tuple[TemplateSettings, ...]:
    ...
    allowed = {"name", "kind", "width", "height", "background_color", "background_image", "background_opacity", "headline", "subtitle", "tagline", "logo", "logo_scale", "logo_anchor", "copy_width", "copy_anchor", "copy_offset_y", "copy_layout", "motto", "motto_rotation_degrees", "character", "character_scale", "character_anchor", "safe_area", "transparent", "output_name"}
...
def _character_anchor(value: Any, location: str) -> str:
    result = _string(value, location)
    if result not in {"center", "right"}:
        raise ConfigurationError(f"{location} must be one of: center, right.")
    return result
